...
class OmicronAgent(RCJSoccerRobot):
    def setup(self):
        ...
        if self.player_id == 1:
            self.attack_fsm.change_state(rs, states.StateAttackChase())
        elif self.player_id == 2:
            self.attack_fsm.change_state(rs, states.StateAttackHover())
        elif self.player_id == 3:
            self.defend_fsm.change_state(rs, states.StateDefendIdle())
        ...
        while self.robot.step(TIME_STEP) != -1:
            if self.is_new_data():
                ...
                if self.rs.agent_id in [1, 2]:
                    self.attack_fsm.update(rs)
                elif self.rs.agent_id == 3:
                    self.defend_fsm.update(rs)
                
                # Update motors
                self.left_motor.setVelocity(rs.out[0][0])
                self.right_motor.setVelocity(rs.out[0][1])
